...
//============================================================================================================
	...
	public AnimationClip aimUpAnimation;
	public AnimationClip aimDownAnimation;
	...
	public float aimAnimationSpeed = 1.0f;
	...
	public Transform VR_CamTransform;
    ...
    public AnimationClip jumpPoseAnimation;	
        */
		...
		if (!aimUpAnimation || !aimDownAnimation)
		{
			_animation = null;
    ...
    {
        Transform cameraTransform = VR_CamTransform;
...
//___________________________________________________________________________________________________________________________________________
      ...
      //  float h = Input.GetAxisRaw("Horizontal");  			//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
		...
		//============================================================================================================
		//	TODO:		ALL ANIMATION SETTING WILL HAVE TO SET IN ONE PLACE ONLY +********************
		//============================================================================================================
		if (Input.GetButton (MainEventManager.StateManager.GameState.myJoyStick.m_forward)) {
			Debug.Log("pressed key XXXXXXXXX");
			v = 1;
		} else {
			v = 0;
		}
		...
		//________________________________AIMING____________________________________________________
		//==========================================================================================
		if (Input.GetButton (MainEventManager.StateManager.GameState.myJoyStick.bt_sniping)) {
//			Debug.Log("Aiming");
			isAiming = true;

		}
		if (Input.GetButtonUp(MainEventManager.StateManager.GameState.myJoyStick.bt_sniping)) {
//			Debug.Log("NOT_Aiming");
			isAiming = false;
		}
		//__________________________________________________________________________________________
		//==========================================================================================   
    ...
    void Update(){
        ...
        if (isControllable)
        {
			...
			//+++++++++++++++++++++++++++++++++++++++++++++++
			if (isAiming) {
//				Debug.Log("Aim so far");
				_characterState = CharacterState.Aiming;
				//=================================================================================
				// SHOOTING
        ...
        if (_animation)
        {
            ...
            else // Have to do this twice once for controller character and second for uncontrolled character
            {
                if (this.isControllable && velocity.sqrMagnitude < 0.001f)
                {

					if (_characterState == CharacterState.Aiming){
						 accelZ = accelUpdate();
						if(accelZ > 0){
							_animation["StandingAimUp"].time = accelZ;
							_animation.Play ("StandingAimUp");
							_animation["StandingAimUp"].speed = 0.0f;
						}else{
							_animation["StandingAimDown"].time = -accelZ;
							_animation.Play ("StandingAimDown");
							_animation["StandingAimDown"].speed = 0.0f;	
						}
					}else{
                   		 _characterState = CharacterState.Idle;
                ...
                else
                {
                    if (_characterState == CharacterState.Idle){
                        _animation.CrossFade(idleAnimation.name);
					}else if (_characterState == CharacterState.Aiming){

						//ACCELZ is being updated throught the photon network
						if(accelZ > 0){
							_animation["StandingAimUp"].time = accelZ;
							_animation.Play ("StandingAimUp");
							_animation["StandingAimUp"].speed = 0.0f;
...
//		Debug.Log("Coroutine shoot");
		...
		if (Physics.Raycast (ray, out hit, 200.0F)) {
			...
			if(hit.transform.tag.Equals("Player")){
				...
				//		hit.transform.GetComponent<ThirdPersonController>().PPIG.Died(); 
				PlayerStats playerStats =  hit.collider.gameObject.GetComponent<PlayerStats>();
				playerStats.GetComponent<PhotonView>().RPC("Damage",PhotonTargets.AllBuffered,20);



			}
//			if(hit.transform.tag.Equals("Crate")){
//				Debug.Log (hit.transform.name + "  hit");
//				//		hit.transform.GetComponent<ThirdPersonController>().PPIG.Died(); 
//				hit.transform.GetComponent<Crate>().hp = -100;
//
//
//
//			}
		...
		// Making sure bool param is set to false while not online
		//yield return 0.7f;
		//this.shooting = false;
